...
public:
    ...
    bool empty() { if (m_head == nullptr) return true; else return false;}
};
...
template<typename T>
void List<T>::erase(List_Iterator<T> it) {
    if (it.m_node == nullptr) return;
    if (it.m_node == m_head) {
        if (it.m_node->next) it.m_node->next->prev = nullptr;
        m_head = it.m_node->next;
    } else {
        it.m_node->prev->next = it.m_node->next;
    }
    if (it.m_node == m_tail) {
        if (it.m_node->prev) it.m_node->prev->next = nullptr;
